...
interface Props {
  params: { storeId: string; taskId: string };
}
    ...
    <div className="p-5 pb-10">
      ...
      <div className="flex items-center gap-3 py-4 mb-6">
        ...
        <h1 className="text-xl font-bold text-gray-900">
          {task.status === "COMPLETED" ? "Count Submitted" : "Submit Count"}
        </h1>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-5">
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="font-medium text-gray-900">{task.category.name}</p>
          </div>
        </div>
      </div>
      ...
      {task.status === "COMPLETED" && task.countRecord ? (
        ...
        <div className="bg-green-50 border border-green-200 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="font-semibold text-green-800">Count Already Submitted</p>
              <p className="text-xs text-green-600">This task has been completed</p>
            </div>
          </div>
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-green-700">Quantity Counted</span>
            ...
            <div className="flex justify-between">
              ...
              <span className="text-green-800">{new Date(task.countRecord.countedAt).toLocaleString()}</span>
            </div>
          </div>
        </div>
      ) : (
